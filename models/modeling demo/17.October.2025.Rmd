...
churndt <- read.csv('/Users/alenslaskovs/Desktop/WHU/PA/GroupWork/MSc_Predictive_Analytics_HS25_Group_Assignment/models/modeling demo/ticdata2000_clean.csv')
...
# --- 0) Build the exact modelling frame once ---
stopifnot("CARAVAN" %in% columns)               # make sure target is in columns
df <- training_set[, columns]

# ensure target is factor with the right positive label
df$CARAVAN <- factor(df$CARAVAN)
if (!all(c("no","yes") %in% levels(df$CARAVAN))) {
  # if levels are reversed or different, coerce to (no, yes)
  df$CARAVAN <- factor(df$CARAVAN, levels = c("no","yes"))
}
pos_lab <- "yes"

# (optional) drop rows with any NA used by the model to keep w aligned
df <- na.omit(df)

# --- 1) Class weights computed on df (not on the larger training_set) ---
pos_n <- sum(df$CARAVAN == pos_lab)
neg_n <- sum(df$CARAVAN != pos_lab)
w <- ifelse(df$CARAVAN == pos_lab, neg_n / pos_n, 1)
...
decTree_T2 <- ctree(fT3, data = df, weights = w, control = ctrl)
...
# --- 3) RPART version (for rpart.plot styling) ---
library(rpart)
library(rpart.plot)

decTree_T2_rpart <- rpart(
  formula  = fT3,
  data     = df,
  weights  = w,
  method   = "class",
...
# Pretty split label wrapper (unchanged)
pretty_map <- c(
  MBERHOOG = "High status",
  MKOOPKLA = "Purchasing power (area)",
  ABRAND   = "Fire insurance (HH)",
  MINKGEM  = "Income (index)",
  MOPLHOOG = "High education",
  MHKOOP   = "Home ownership",
...
pretty_splits <- function(x, labs, digits, varlen, faclen) {
  labs <- as.character(labs)
  for (nm in names(pretty_map)) {
    labs <- gsub(paste0("\\b", nm, "\\b"), pretty_map[[nm]], labs, perl = TRUE)
  }
  vapply(labs, function(s) paste(strwrap(s, width = 18), collapse = "\n"), character(1))
}
...
rpart.plot::rpart.plot(
  ...
  varlen          = 0,
  digits          = -3,
  main            = "Caravan Propensity — Cost-Sensitive Tree",
  uniform         = TRUE,
  compress        = FALSE,
  minbranch       = 0.85,
  tweak           = 1.35,
