...
# 4) Pretty labels + Sortierung innerhalb jedes Facets
pretty_map <- c(
...
rng  <- max(abs(top5$z_mean), na.rm = TRUE)
pad  <- 0.2
xlim <- c(-(rng + pad), (rng + pad))

ggplot(top5, aes(x = z_mean, y = feature_pretty)) +
  geom_vline(xintercept = 0, linewidth = 0.4, linetype = 3, color = "grey50") +
  geom_segment(aes(x = 0, xend = z_mean, y = feature_pretty, yend = feature_pretty),
               linewidth = 1) +
  geom_point(size = 2.8) +
  facet_wrap(~ cluster_lab, ncol = 2) +
  scale_x_continuous(limits = xlim) +
  labs(title = "Top 5 differentiators per cluster (z vs overall mean)",
       x = "Standardized deviation (z)", y = NULL) +
...
ggplot(top5, aes(x = z_mean, y = feature_pretty)) +
  ...
  geom_segment(aes(x = 0, xend = z_mean, y = feature_pretty, yend = feature_pretty),
               linewidth = 1) +
  geom_point(size = 2.8) +
  facet_wrap(~ cluster_lab, ncol = 1) +   # <<-- alles untereinander
  scale_x_continuous(limits = xlim) +
  labs(title = "Top 5 differentiators per cluster (z vs overall mean)",
       x = "Standardized deviation (z)", y = NULL) +
  theme_minimal(base_size = 12) +
...
# Erwartet ein 'top5' aus dem vorherigen Chunk mit: cluster_ok, feature_pretty, z_mean
stopifnot(all(c("cluster_ok","feature_pretty","z_mean") %in% names(top5)))

# Panels untereinander + Reihenfolge C1, C2, C3
top5 <- top5 %>%
  mutate(
    cluster_lab = if (!"cluster_lab" %in% names(.))
      forcats::fct_relabel(as.factor(cluster_ok), ~ paste0("C", .x)) else cluster_lab,
    cluster_lab = forcats::fct_relevel(cluster_lab, "C1","C2","C3"),
    z_lab = sprintf("%.2f", z_mean)
  )

# Dynamische Achsen-Polsterung, damit nichts abgeschnitten wird
min_z <- min(top5$z_mean, na.rm = TRUE)
max_z <- max(top5$z_mean, na.rm = TRUE)
range_z <- max_z - min_z
pad <- max(0.15, 0.08 * range_z)  # mind. 0.15 Puffer
xlim <- c(min_z - pad, max_z + pad)

ggplot(top5, aes(x = z_mean, y = feature_pretty)) +
  # Null-Linie
  geom_vline(xintercept = 0, linewidth = 0.4, linetype = 3, color = "grey50") +
  # Segment (links/rechts vom Nullpunkt)
  geom_segment(aes(x = 0, xend = z_mean, y = feature_pretty, yend = feature_pretty),
               linewidth = 1) +
  # Punkt
  geom_point(size = 2.8) +
  # Wert-Label mit richtungsabhängigem Versatz (damit nicht am Rand klebt)
  geom_text(aes(label = z_lab, x = z_mean,
                hjust = ifelse(z_mean >= 0, -0.2, 1.2)),
            vjust = 0.5, size = 3) +
  facet_wrap(~ cluster_lab, ncol = 1, scales = "free_y") +
  ...
  theme(
    ...
    strip.text = element_text(face = "bold"),
    axis.text.y = element_text(hjust = 1),
    # mehr Platz links/rechts, damit Labels & Punkte nicht abgeschnitten werden
...
# -------- 1) Gleiche Merkmale für alle Panels (Top-k global) -----------------
top_k <- 5  # <- Anzahl Merkmale, die in allen drei Panels gezeigt werden sollen

# Top-k Features nach max |z| über alle Cluster
feature_order <- centers_long %>%
  group_by(feature) %>%
  summarise(max_abs = max(abs(z_mean), na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(max_abs)) %>%
  slice_head(n = top_k) %>%
  pull(feature)

# -------- 2) Plot-Data bauen (nur diese Features, gleiche Reihenfolge überall)
plot_df <- centers_long %>%
  filter(feature %in% feature_order) %>%
  mutate(
    # hübsche Labels
    feature_pretty = dplyr::recode(feature, !!!pretty_map, .default = feature),
    # globale Reihenfolge nach "feature_order" (damit in allen Panels gleich)
    feature_pretty = factor(
      feature_pretty,
      levels = rev(dplyr::recode(feature_order, !!!pretty_map, .default = feature_order))
    ),
    # C1/C2/C3 Labels
    cluster_lab = fct_relabel(as.factor(cluster_ok), ~ paste0("C", .x)),
    # Vorzeichen für Farbe
    sign = ifelse(z_mean >= 0, "pos", "neg"),
    z_lab = sprintf("%.2f", z_mean)
  )

# -------- 3) Einheitliche, symmetrische x-Achse (nichts abschneiden) --------
min_z <- min(plot_df$z_mean, na.rm = TRUE)
max_z <- max(plot_df$z_mean, na.rm = TRUE)
rng   <- max(abs(c(min_z, max_z)))
pad   <- 0.15
xlim  <- c(-rng - pad, rng + pad)

# Farbpalette: negativ rot, positiv grün
pal <- c(neg = "#D73027", pos = "#1A9850")

ggplot(plot_df, aes(x = z_mean, y = feature_pretty, colour = sign)) +
  # Null-Linie
  geom_vline(xintercept = 0, linewidth = 0.4, linetype = 3, color = "grey50") +
  # Lollipop (Segment + Punkt)
  geom_segment(aes(x = 0, xend = z_mean, y = feature_pretty, yend = feature_pretty),
               linewidth = 1, lineend = "round") +
  geom_point(size = 3) +
  # Wert-Label in der jeweiligen Farbe, leicht vom Punkt weg versetzt
  geom_text(aes(label = z_lab,
                hjust = ifelse(z_mean >= 0, -0.2, 1.2)),
            vjust = 0.5, size = 3) +
  facet_wrap(~ cluster_lab, ncol = 1) +   # alles untereinander
  scale_x_continuous(limits = xlim, expand = expansion(mult = c(0, 0))) +
  scale_colour_manual(values = pal, labels = c(neg = "below mean", pos = "above mean"),
                      name = NULL) +
  coord_cartesian(clip = "off") +
  labs(title = "Top differentiators (same features across clusters)",
       subtitle = paste0("z vs overall training mean • k = 3 • Top-", top_k, " by global |z|"),
       x = "Standardized deviation (z)", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.major.y = element_blank(),
...
# 1) Fixierte Features (allen Panels gleich)
fixed_features <- c(
  "MBERHOOG",  # High status
  "MKOOPKLA",  # Purchasing power
  "ABRAND",    # Fire insurance (HH)
...
# 2) Plot-Daten (gleiche Merkmale in allen Panels)
plot_df <- centers_long %>%
  dplyr::filter(feature %in% fixed_features) %>%
  dplyr::mutate(
    feature_pretty = dplyr::recode(feature, !!!pretty_map, .default = feature),
    feature_pretty = factor(
      feature_pretty,
      levels = rev(dplyr::recode(fixed_features, !!!pretty_map, .default = fixed_features))
    ),
    cluster_lab = forcats::fct_relabel(as.factor(cluster_ok), ~ paste0("C", .x)),
    sign = ifelse(z_mean >= 0, "pos", "neg")
  )

# 3) Einheitliche, symmetrische x-Achse & Farben
rng  <- max(abs(plot_df$z_mean), na.rm = TRUE)
pad  <- 0.15
xlim <- c(-rng - pad, rng + pad)
pal  <- c(neg = "#D73027", pos = "#1A9850")  # rot / grün

# make THIS chunk's figure taller (inches)
knitr::opts_current$set(fig.width = 15, fig.height = 30, out.height = "16in")


# 4) Plot (ohne Zahlenlabels)
ggplot(plot_df, aes(x = z_mean, y = feature_pretty, colour = sign)) +
  geom_vline(xintercept = 0, linewidth = 0.4, linetype = 3, color = "grey50") +
  geom_segment(aes(x = 0, xend = z_mean, y = feature_pretty, yend = feature_pretty),
               linewidth = 1, lineend = "round") +
  geom_point(size = 3) +
  facet_wrap(~ cluster_lab, ncol = 1) +
  scale_x_continuous(limits = xlim, expand = expansion(mult = c(0, 0))) +
  scale_colour_manual(values = pal, labels = c(neg = "below mean", pos = "above mean"),
                      name = NULL) +
  coord_cartesian(clip = "off") +
  labs(title = "Differentiators (same 7 features across clusters)",
       subtitle = "z vs overall training mean • k = 3",
       x = "Standardized deviation (z)", y = NULL) +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.major.y = element_blank(),
...
ggplot(plot_df, aes(x = z_mean, y = feature_pretty, colour = sign)) +
  ...
  scale_colour_manual(values = pal,
                      labels = c(neg = "below mean", pos = "above mean"),
                      name = NULL) +
  coord_cartesian(clip = "off") +
  labs(
    title = "Differentiators (same 7 features across clusters)",
    subtitle = "z vs overall training mean • k = 3",
  ...
  theme(
    ...
    strip.text   = element_text(face = "bold"),
    axis.text.y  = element_text(hjust = 1),
    plot.margin  = margin(t = 50, r = 30, b = 10, l = 28),
    legend.position = "top",
    # a bit more vertical space between the three facet rows:
